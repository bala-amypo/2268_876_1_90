...
@Tag(
        name = "Duplicate Rule Controller",
        description = "Manage duplicate detection rules"
)
public class DuplicateRuleController {
    ...
    @PostMapping("/")
    public ResponseEntity<DuplicateRule> createRule(
            @RequestBody DuplicateRule rule) {

        return ResponseEntity.ok(
                duplicateRuleService.createRule(rule)
        );
    }
    ...
    public ResponseEntity<List<DuplicateRule>> getAllRules() {
        return ResponseEntity.ok(
                duplicateRuleService.getAllRules()
        );
    }

    @GetMapping("/{id}")
    public ResponseEntity<DuplicateRule> getRule(
            @PathVariable Long id) {

        return ResponseEntity.ok(
                duplicateRuleService.getRule(id)
        );
    }
}
